...
public sealed class GetAllOrdersMapper : IGetAllOrdersMapper
{
    public GetAllOrdersExportOrder Map(OrderEntity order)
    {
        ...
        if (order.ProductQuantities == null || order.ProductQuantities.Count == 0 || order.ProductQuantities.Any(x => x.Quantity <= 0))
            throw new ArgumentNullException(nameof(order.ProductQuantities));

        var productQuantities = order
            .ProductQuantities
            .Select(x => new GetAllOrdersExportOrderProductQuantity(x.Product.ProductId, x.Quantity))
            .ToList();
